...
type SaveSnippetContext = (
  code: string,
  snippetName: string,
  template: Languages | string,
) => number;
type RenameSnippetContext = (
  id: number,
  data: { code: string; name: string },
) => void;
type UpdateSnippetContext = (
  id: number,
  data: { code: string; name: string },
) => void;
...
type GetSnippetDataByViewParamsContext = (data: {
  username: string;
  slug: string;
}) => FetchedSnippet;
type HasViewSnippetParamsContext = (data: {
  username: string;
  slug: string;
}) => string;
type GenEmbedSnippetLinkContext = (
